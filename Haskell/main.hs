...
funcHead = head [5,4..1] :: Integer
--operator 'tail' takes a list and returns its tail
--operator 'tails' does basically the same
funcTail = tail [5,4..1] :: [Integer]
funcTails = tails [5,4..1] :: [[Integer]]
--operator 'last' takes a list and returns the last element
funcLast = last [5,4..1] :: Integer
--operator 'init' takes a list and returns everything except last
--operator 'inits' does basically the same
funcInit = init [5,4..1] :: [Integer]
funcInits = inits [5,4..1] :: [[Integer]]
--operator 'lines' takes some input and separates every new line in a string
--operator 'unlines' does the opposite
funcLines = lines "go wash \nyo hair\nboy" :: [[Char]]
--operator 'words' takes a string and makes a list of words
--operator 'unword' does the opposite
funcWords = words "hello there" :: [[Char]]
--operator '\\' differenciates
funcDiff = [1..30] \\ [1..20]
--operator 'union' unites two lists
funcUnion = union "hey man" "man what's up"
--opeartor 'intersect' returns only elements that are present in both lists
funcIntersect = intersect [1..7] [5..10]
--operator 'insert' inserts an element in a list correctly
funcInsert = insert 3 [1,2,4,5]
--operator 'length' takes a list and returns its length
funcLength xs = length xs :: Int
...
funcNull xs = null xs :: Bool
--operator 'nub' takes a list and weeds out the duplicates
funcNub xs = nub xs :: [Integer]
{-operator 'elem' takes a thing and searches a list to see if that element appears in the list,answers are: True False
operator 'elemIndex' takes a thing and searches a list to see if that element apperas in the list,answers are the position of the element in the list
operator 'elemIndices' does the same like 'elemIndex' but returns indices -}
funcElem :: (Foldable t, Eq a) => a -> t a -> Bool
funcElem x xs = elem x xs
funcElemIndex :: Eq a => a -> [a] -> Maybe Int
funcElemIndex x xs = elemIndex x xs
funcElemIndices :: [Int]
funcElemIndices = elemIndices ' ' "Where are the spaces ?"
...
--operator 'findIndex' work like find
--operator 'findIndices' works like findIndex
funcFind = find (>4)[1..10] :: Maybe Integer
funcFindIndex = findIndex (==4) [5,3,2,1,6,4] :: Maybe Int
funcFindIndices = findIndices (==4) [1,4,3,4,1,3,5] :: [Int]
...
--operator 'groupBy' groups like opeartor 'group' but it comes with a equality function
listGroupby = [1,-3,3,1]
funcGroupBy = groupBy (\x y -> (x > 0) == (y > 0)) listGroupby

-- List comprehension
func22 = [x*2 | x <- [1..10]] :: [Integer]
-- same but with a condition
func23 = [x*2 | x <- [1..20], x*2 >= 12] :: [Integer]
...
-- nouns and adj joke :-)
nouns = ["hobo","frog","pope"] :: [[Char]]
adjectives = ["lazy","grouchy","scheming"] :: [[Char]]
funcnounadj :: [[Char]]
